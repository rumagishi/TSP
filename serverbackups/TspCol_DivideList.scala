 ...
 * updated on 8/9
 *
 * parメソッドを使わないで，リストをdiv(145行付近)分割することで仕事を小さくしようっていう考え．
 * でも全然遅い．
 *
 * */
...
object TspCol_DivideList {
  ...
  def main(args: Array[String]): Unit = {
    if(args.length != 3) {
        ...
        loop {
          react {
            ...
            case candidates: List[List[Int]] => {
              //println("===findBest===")
              reply(retRou2(candidates)) //List[Int]
            }
          }
        }
      }
        ...
        loopWhile(running) {
          react {
            case route: List[Int] => {
              ...
              val best = for(i <- 0 until div) yield (findBest !? slicedList(i).toList).asInstanceOf[List[Int]]
      ...
      while(state != samplepoint.getState) { /*println("wait")*/ }
      ...
      //val results = for (i <- 0 until routes.length) yield (samplepoint !? routes(i)).asInstanceOf[List[Int]]
      //while(state != samplepoint.getState) { println("wait") }
      sys.exit(0)
    }
  }
